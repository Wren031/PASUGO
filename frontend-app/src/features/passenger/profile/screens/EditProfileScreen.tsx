...
import { useUpdatePassengerProfile } from '../hooks/usePassengerProfile';
...
export function EditProfileScreen() {
  ...
  const updateProfile = useUpdatePassengerProfile(user?.id ?? '');
    ...
    <Screen>
      ...
      <ScrollView className="flex-1 px-4 pb-8" showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        <View className="mt-4 rounded-2xl border border-line bg-white p-4">
          ...
          <View className="mt-4">
            <Input label="Email" value={email} onChangeText={setEmail} placeholder="juan@email.com" keyboardType="email-address" autoCapitalize="none" />
          </View>
        </View>
